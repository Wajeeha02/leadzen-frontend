  ...
  return (
    <div className="h-[1rem] sm:h-[1.5rem] md:h-[2rem] lg:h-[1.2rem] flex items-center justify-between p-8 sm:p-10 md:p-14 lg:p-12 pt-10 scroll-smooth">
      ...
      <div className="hidden lg:flex md:flex font-poppins py-4">
        <nav className="flex space-x-2 sm:space-x-4 md:space-x-8 lg:space-x-24 ml-[0.25rem] sm:ml-[1.75rem] md:ml-[2rem] lg:ml-16 2xl:ml-96">
      ...
      {/* Mobile nav overlay */}
      {isMenuOpen && (
        <div className="lg:hidden md:hidden fixed inset-0 top-[4rem] left-0 w-full h-[calc(100vh-4rem)] bg-bluedark bg-opacity-95 text-white z-10 flex justify-center items-center">
          <nav className="flex flex-col items-center space-y-6" onClick={closeMenu}>
            <a href="/" className={`${textColor} text-lg font-medium`}>Home</a>
            <a href="/about" className={`${textColor} text-lg font-medium`}>About</a>
            <a href="/dashboard" className={`${textColor} text-lg font-medium`}>Dashboard</a>
